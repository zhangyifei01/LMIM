...
class AloneImageLmdb(Dataset):
  ...
  def __getitem__(self, index):
    ...
    img_ = np.asarray(img)
    img_ = cv2.resize(img_, dsize=(128, 32), interpolation=cv2.INTER_CUBIC)
    # print(img_.shape) # (32, 128, 3)
    hog_features_r = hog(img_[:,:,0], orientations=9, pixels_per_cell=(4, 4), cells_per_block=(1, 1), block_norm='L2', feature_vector=False)
    hog_features_g = hog(img_[:,:,1], orientations=9, pixels_per_cell=(4, 4), cells_per_block=(1, 1), block_norm='L2', feature_vector=False)
    hog_features_b = hog(img_[:,:,2], orientations=9, pixels_per_cell=(4, 4), cells_per_block=(1, 1), block_norm='L2', feature_vector=False) #visualize=True
    # print(hog_features_b.shape) # (8, 32, 1, 1, 9)
    hog_features = np.concatenate([hog_features_r,hog_features_g,hog_features_b], axis=-1)
    # print(hog_features.shape) # (8, 32, 1, 1, 27)
    hog_features = rearrange(hog_features, '(ph dh) (pw dw) ch cw c -> ph pw (dh dw ch cw c)', ph=8, pw=32)
    ...
    if self.use_aug:
      ...
      return (img, vis_mask, hog_features), np.ones(1), np.ones(1)