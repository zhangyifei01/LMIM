...
class RandomColMaskingGenerator:
    def __init__(self, input_size, mask_ratio, aug_ratio=0., num_view=1):
        self.num_view = num_view
        ...
        if not isinstance(input_size, tuple):
            input_size = (input_size,) * 2
        self.height, self.width = input_size
        self.num_col_mask = int(mask_ratio * self.width)

        self.num_patches = self.height * self.width
        self.num_mask = self.num_col_mask * self.height
    ...
    def __call__(self):
        ...
        mask[:, :self.num_col_mask] = 0

        columns = np.arange(mask.shape[1])
        np.random.shuffle(columns)

        mask = mask[:, columns].flatten()
